...
func TestValidateOperationInput_ComposedSchemas(t *testing.T) {
	tests := []struct {
		...
		{name: "oneOf overlapping matches", schema: &SchemaSpec{OneOf: []*SchemaSpec{{Type: "number"}, {Type: "integer"}}}, body: `1`, want: `matches 2 schemas in oneOf`},
		{name: "oneOf partial constraints", schema: &SchemaSpec{OneOf: []*SchemaSpec{{Type: "string", StaticValidationIncomplete: true}, {Type: "string", StaticValidationIncomplete: true}}}, body: `"cat"`},
