...
func TestRenderModule_EmitsRequestBodyEnvelope(t *testing.T) {
		...
		RequestBody: &runtime.RequestBody{
			...
			Schema: &runtime.SchemaSpec{
				...
				Properties: map[string]*runtime.SchemaSpec{
					...
					"kind":  {Type: "string", StaticValidationIncomplete: true},
					"ratio": {Type: "number", AcceptStringEncodedNumber: true},
					"state": {Type: "string", AcceptIntegerEnum: true},
					"tags":  {Type: "array", AcceptSingletonArray: true, Items: &runtime.SchemaSpec{Type: "string"}},
				},
	...
	for _, want := range []string{
		...
		`AcceptIntegerID: true`,
		`AcceptSingletonArray: true`,
		`StaticValidationIncomplete: true`,
		`SchemaDefinitions: map[string]*runtime.SchemaSpec{`,
		`"Node":`,
		`"input"`,
		`"name"`,
