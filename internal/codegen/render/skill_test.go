...
func TestRenderSkillDirectory_GeneratesSkillStructure(t *testing.T) {
	...
	for _, want := range []string{"## Search", "## Full Catalog", "## Command Detail", "## Sensitive Flags", "## Schema", "input_modes", "body.runtime_schema", "body.schema.definitions", "x-lathe-accepts-singleton-array", "--<flag>-env", "--<flag>-file", "--<flag>-stdin", "--set-str", "-o json", "error.http", "pause exits zero"} {
