...
func requestBodyLiteral(body *runtime.RequestBody) string {
	...
	if len(body.SchemaDefinitions) > 0 {
		fmt.Fprintf(&b, "SchemaDefinitions: %s,", schemaDefinitionsLiteral(body.SchemaDefinitions))
	}
...
func schemaDefinitionsLiteral(definitions map[string]*runtime.SchemaSpec) string {
	var b strings.Builder
	b.WriteString("map[string]*runtime.SchemaSpec{")
	...
	for _, key := range keys {
		fmt.Fprintf(&b, "%q: ", key)
		writeSchemaLiteral(&b, definitions[key])
		b.WriteByte(',')
	}
	b.WriteByte('}')
	return b.String()
...
func writeSchemaLiteral(b *strings.Builder, s *runtime.SchemaSpec) {
	...
	writeBoolField(b, "StaticValidationIncomplete", s.StaticValidationIncomplete)
...
var moduleTmpl = template.Must(template.New("gen").Funcs(template.FuncMap{
	...
	"schemaDefinitionsLiteral": schemaDefinitionsLiteral,
...
{{- range $op := .Ops}}
	{
		...
		{{- if $op.RequestBody}}
			RequestBody: &runtime.RequestBody{
				...
				{{- end}}
				{{- if $op.RequestBody.SchemaDefinitions}}
				SchemaDefinitions: {{schemaDefinitionsLiteral $op.RequestBody.SchemaDefinitions}},
				{{- end}}
