...
func parseSDLWithConfig(t *testing.T, sdl string, cfg *sourceconfig.GraphQLConfig) (*rawir.RawModule, error) {
	t.Helper()
	dir := t.TempDir()
	testutil.NoError(t, os.WriteFile(filepath.Join(dir, "schema.graphql"), []byte(sdl), 0o644))
...
func queryDocument(t *testing.T, op rawir.RawOperation) string {
	...
	testutil.NoError(t, json.Unmarshal([]byte(op.RequestBody.Template), &env))
...
func TestParse_ExposesOnlyPolicyMatchedOps(t *testing.T) {
	...
	testutil.Require(t, len(mod.Operations) == 3, "ops = %d, want 3 (got %v)", len(mod.Operations), ops)
...
func TestParse_BakesEnvelopeAndVariables(t *testing.T) {
	...
	testutil.Require(t, create.Method == "POST" && create.Path == "/graphql", "http = %s %s, want POST /graphql", create.Method, create.Path)
	testutil.Require(t, create.RequestBody != nil && create.RequestBody.MergePath == "variables", "envelope = %+v", create.RequestBody)

	var env map[string]any
	testutil.NoError(t, json.Unmarshal([]byte(create.RequestBody.Template), &env))
	q, _ := env["query"].(string)
	for _, want := range []string{
		...
		testutil.Check(t, strings.Contains(q, want), "baked query missing %q:\n%s", want, q)
	}
	if _, ok := env["variables"].(map[string]any); !ok {
		t.Errorf("variables envelope missing/!object: %#v", env["variables"])
	}

	testutil.Require(t, len(create.Parameters) == 1, "createApp params = %+v, want 1", create.Parameters)
	p := create.Parameters[0]
	testutil.Require(t, p.Name == "name" && p.In == "variable" && p.Required && p.Type == "string", "name variable = %+v", p)

	apps := ops["console_apps"]
	testutil.Require(t, len(apps.Parameters) == 1, "apps params = %+v, want 1", apps.Parameters)
...
func TestParse_GeneratesValidGraphQLDocuments(t *testing.T) {
	...
	testutil.Require(t, err == nil, "reload schema: %v", err)
...
func TestParse_NormalizesToDistinctCommands(t *testing.T) {
	...
	testutil.Require(t, len(specs) == 2, "specs = %d, want 2", len(specs))
	...
	for _, use := range []string{"apps", "create-app"} {
		s, ok := byUse[use]
		testutil.Require(t, ok, "missing command %q (got %v)", use, byUse)
		testutil.Check(t, s.Method == "POST" && s.PathTpl == "/graphql", "%s: http = %s %s, want POST /graphql", use, s.Method, s.PathTpl)
		testutil.Check(t, s.RequestBody != nil && s.RequestBody.MergePath == "variables" && s.RequestBody.Template != "", "%s: envelope = %+v", use, s.RequestBody)
	}
	create := byUse["create-app"]
	testutil.Require(t, len(create.Params) == 1, "create-app params = %+v, want 1", create.Params)
...
func TestParse_AppliesGraphQLGroupAndOutputPolicy(t *testing.T) {
	...
	testutil.Require(t, apps.Group == "Applications", "raw group = %q, want Applications", apps.Group)
	testutil.Require(t, apps.Output != nil && apps.Output.ListPath == "data.apps.nodes" && strings.Join(apps.Output.DefaultColumns, ",") == "id,name", "raw output = %+v", apps.Output)
	specs := normalize.Normalize(mod)
	testutil.Require(t, len(specs) == 1, "specs = %d, want 1", len(specs))
	testutil.Require(t, specs[0].Group == "Applications", "normalized group = %q, want Applications", specs[0].Group)
	testutil.Require(t, specs[0].Output.ListPath == "data.apps.nodes" && strings.Join(specs[0].Output.DefaultColumns, ",") == "id,name", "normalized output = %+v", specs[0].Output)
}
...
`
	...
	testutil.Require(t, op.Output != nil && op.Output.Pagination != nil, "raw output pagination = %+v", op.Output)
	...
	specs := normalize.Normalize(mod)
	testutil.Require(t, len(specs) == 1 && specs[0].Output.Pagination != nil, "normalized pagination missing: %+v", specs)
...
func TestParse_AppliesGraphQLSelectionPolicy(t *testing.T) {
	...
	testutil.Require(t, strings.Contains(q, "id"), "query should keep unpruned scalar field:\n%s", q)
	for _, no := range []string{"name", "owner", "email"} {
		testutil.Require(t, !strings.Contains(q, no), "query should not include %q under selection policy:\n%s", no, q)
	}
}
...
func TestParse_FailsClosedOnAmbiguousGraphQLGroupPolicy(t *testing.T) {
	...
	testutil.Require(t, err != nil, "expected fail-closed error for ambiguous group policy")
	testutil.Check(t, strings.Contains(err.Error(), "multiple graphql.groups"), "error = %v, want to mention multiple graphql.groups rules", err)
}
...
func TestParse_FailsClosedOnAmbiguousGraphQLOutputPolicy(t *testing.T) {
	...
	_, err := parseSDLWithConfig(t, consoleSDL, cfg)
	testutil.Require(t, err != nil, "expected fail-closed error for ambiguous output policy")
	testutil.Check(t, strings.Contains(err.Error(), "multiple graphql.output"), "error = %v, want to mention multiple graphql.output rules", err)
}
...
`
	...
	testutil.Require(t, err != nil, "expected fail-closed error: Thing has no auto-selectable fields")
	testutil.Check(t, strings.Contains(err.Error(), "no selectable fields"), "error = %v, want to mention no selectable fields", err)
}
...
`
	...
	testutil.Require(t, err != nil, "expected fail-closed error after selection policy prunes every field")
	testutil.Check(t, strings.Contains(err.Error(), "no selectable fields"), "error = %v, want to mention no selectable fields", err)
}
...
`
	...
	testutil.Require(t, len(specs) == 1, "specs = %d, want 1", len(specs))
	p := specs[0].Params
	testutil.Require(t, len(p) == 1 && p[0].Name == "weight" && p[0].GoType == "float64" && p[0].Required, "weight variable = %+v", p)
}
...
`
	...
	testutil.Require(t, len(got) == len(want), "params = %#v, want %#v", got, want)
	for name, wantType := range want {
		testutil.Check(t, got[name] == wantType, "%s GoType = %q, want %q", name, got[name], wantType)
	}
}
...
`
	...
	testutil.Require(t, flags["input.name"] == "input-name" && flags["input.region"] == "input-region", "flags = %#v", flags)
}
...
`
	...
	testutil.Require(t, err == nil, "optional non-scalar argument should be allowed: %v", err)
	apps := byID(mod.Operations)["console_apps"]
	testutil.Require(t, len(apps.Parameters) == 1, "apps params = %+v, want 1", apps.Parameters)
	...
	var env map[string]any
	testutil.NoError(t, json.Unmarshal([]byte(apps.RequestBody.Template), &env))
...
`
	mod, err := parseSDL(t, sdl, nil, []string{"createApp"})
	testutil.Require(t, err == nil, "required complex input field should use body schema: %v", err)
	create := byID(mod.Operations)["console_createApp"]
	testutil.Require(t, len(create.Parameters) == 1, "params = %+v, want only scalar leaf flag", create.Parameters)
	if p := create.Parameters[0]; p.Name != "input.name" || !p.Required {
		t.Fatalf("input.name param = %+v", p)
	}
	schema := create.RequestBody.Schema
	testutil.Require(t, schema != nil && schema.Properties["input"] != nil, "body schema = %+v, want input object", schema)
...
`
	...
	testutil.Require(t, err != nil, "expected fail-closed error for query/mutation name collision")
	testutil.Check(t, strings.Contains(err.Error(), "collision"), "error = %v, want to mention collision", err)
}
